...
from muffin_shop.helpers.main.json import (
    get_json_archive_path,
    insert_to_json_archive,
    remove_from_json_archive,
)
...
def pretty_time_formatter(timestamp):
    timestamp = datetime.datetime.strptime(
        timestamp.split(".", 1)[0], "%Y-%m-%d %H:%M:%S"
    )
...
def blog_index():
    ...
    return render_template(
        ...
        page_range=[]
        if last_page == 0
        else reversed(range(max(last_page - 11, 1), last_page + 1)),
...
def blog_new_post():
    ...
    if form.validate_on_submit():
        ...
        success = insert_to_json_archive(
            f"{os.environ['BLOG_PATH']}/posts.json", new_post
        )
        flash(
            "Created new post! ✔️" if success else "Error",
...
def blog_edit_post(post_id):
    ...
    filled_form = {"content": posts[post_id]["content"]}
